...
# - read single pickled data frame, using the proper naming convention
# - read all pickled data frame, using the proper naming convention
...
def read_df(name='all_cities', size='full', suffix='raw'):
    """Read presaved dataframe."""

    root = '/home/'
    data = 'data'

    df = pd.read_pickle(os.path.join(os.path.join(root, data), 'df_%s_%s_%s.pkl' % (name, size, suffix)))

    return df

def read_data_per_city(names, size='full', suffix=''):
    """Read presaved data around each city."""
    ...
    for name in names:
        dfs_full.append(
            pd.read_pickle(os.path.join(os.path.join(root, data), 'df_%s_%s_%s.pkl' % (name, size, suffix))))
